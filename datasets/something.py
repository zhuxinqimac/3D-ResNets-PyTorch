...
class Something(data.Dataset):
    ...
    def _get_normal_plus_shuffle(self, index):
        ...
        if self.spatial_transform is not None:
            ...
            abnorm_clip = [self.spatial_transform(img) for img in abnorm_clip]
        norm_clip = torch.stack(norm_clip, 0).permute(1, 0, 2, 3)
        abnorm_clip = torch.stack(abnorm_clip, 0).permute(1, 0, 2, 3)

        target = self.data[index]
        if self.target_transform is not None:
            target = self.target_transform(target)

        return [norm_clip, abnorm_clip, path], target

    def _get_normal_inf(self, index):
        path = self.data[index]['video']
    ...
    def __getitem__(self, index):
        ...
        if self.score_sens_mode:
            return self._get_normal_plus_shuffle(index)
        elif self.score_inf_mode:
            return self._get_normal_inf(index)
        path = self.data[index]['video']
